...
router.get('/', isAuthenticated, mgmtCheck, async function(req, res, next) {
  ...
  var tokenClaims = req.session.account.idTokenClaims;
  var manager = tokenClaims.preferred_username;
  //get direct reports from API
  var directReports = await fetch(GRAPH_DIRECT_REPORTS, req.session.accessToken)
  ...
  for(e in emps){
    var id = emps[e]["mail"].split("@")[0].toLowerCase(); //all lowercase
    //console.log(id);
